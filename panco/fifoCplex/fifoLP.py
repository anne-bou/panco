...
from panco.fifoCplex.treeLP import TreeLPCpx
...
from panco.fifoCplex.cplex_lex import lex_cplex, cplex_solve, sol_extraction, sol_extraction_burst_lp
...
class FifoLPCpx:
    def __init__(self, network: Network, list_edges=None, polynomial=True, sfa=False, tfa=False, filename="fifo_cplex.lp"):
        """
        ...
        :param tfa: True for inclusion of the tfa delay constraints (for polynomial method only)
        ...
        if self.tfa:
            self.tfa_delays = AdmTfaCpx(network, filename="fifoLP_tfacpx.lp").delay_servers
            if network.num_servers > 0 and self.tfa_delays[0] == np.inf:
                self.tfa = False
        self.sfa = sfa
        self.sfa_delays = None
        if self.sfa:
            self.sfa_delays = SfaLPCpx(network).all_delays
    ...
    def lp_constraint_flow(self, foi: int, file):  # foi flow of the decomposition
        ...
        if self.polynomial:
            if self.tfa_delays is not None:
                ...
                sub_tfa_delays = None
            lp = PLPConstraintsCpx(net, new_foi, foi + 1, list_flows, None, sub_tfa_delays)
        else:
    ...
    def lp_constraints(self, file):
        ...
        file.write('maximize \n obj: ')
        file.write(' x0')
        for i in range(1, self.forest.num_flows):
    ...
    def lp_program(self) -> np.ndarray:
        ...
        tab_bursts = sol_extraction_burst_lp(dict_sol, self.forest.num_flows)
        # print(self.forest)
        # return (self.forest)
        # file.close()
    ...
    def update_sigma(self, f, sigma):
        ...
        sigma[f] = TreeLPCpx(sub_net, new_f, self.polynomial, self.sfa, self.tfa, self.filename).backlog
    ...
    def all_delays(self) -> List[float]:
        ...
        while i < ff.num_flows:
            ...
            d += TreeLPCpx(tree, foi, self.polynomial, self.sfa, self.tfa, "fifoLP_delay_cplex.lp").delay
            i += 1
            if i in self.list_first:
                tab_delays += [d]
                d = 0
    ...
    def delay(self, foi: int) -> float:
        ...
        while (foi < self.network.num_flows - 1 and i < self.list_first[foi + 1]) or \
            ...
            delay += TreeLPCpx(tree, foi1, self.polynomial, self.sfa, self.tfa, filename="fifoLP_delay_cplex.lp").delay
            i += 1
        return delay
