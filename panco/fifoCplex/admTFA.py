...
# from cplex_lex import dict_sol, tab_delays, dict_bursts_shaping
...
from panco.fifoCplex.cplex_lex import cplex_solve, lex_cplex, sol_extraction, extract_obj
...
**[Bou] Anne Bouillard, Admission Shaping With Network Calculus. IEEE Networking Letter 6(2): 115-118 (2024)**
...
class AdmTfaCpx:
    ...
    For cyclic networks, it computes the same as [TLM19], without this same assumption,
    ...
    """
    def __init__(self, network: Network, filename="tfa_new_cplex.lp"):
        ...
        for i in range(self.network.num_flows):
            for (l, j) in enumerate(self.network.path[i]):
                if j == self.network.path[i][0]:
                    ...
                    for k, tb in enumerate(self.network.flows[i].arrival_curve):
                        file.write('x{0}l{1}s{2} - x{0}l{1}s{3} - {4} d{3} = 0\n'.format(i, k, j,
    ...
    def tfa_constraints_server(self, file):
        ...
        for j in range(self.network.num_servers):
            ...
            for i in self.network.flows_in_server[j]:
                for k, tb in enumerate(self.network.flows[i].arrival_curve):
                    # print(type(tb))
                    file.write('f{0}s{1}u{1} -  x{0}l{2}s{1} - {3} u{1}<= 0\n'.format(i, j, k, tb.rho))
            for h in self.network.predecessors[j]:
                for tb in self.network.servers[h].max_service_curve:
                    file.write('f{0}s{1}u{1}'.format(self.network.edges[(h, j)][0], j))
                    for i in self.network.edges[(h, j)][1:]:
                        file.write('+ f{0}s{1}u{1}'.format(i, j))
                    file.write(' - {1} u{2}<= {0}\n'.format(tb.sigma, tb.rho, j))
            # file.write("0")
            file.write('f{0}s{1}u{1}'.format(self.network.flows_in_server[j][0], j))
            for i in self.network.flows_in_server[j][1:]:
                file.write('+ f{0}s{1}u{1}'.format(i, j))
            file.write('- a{0}u{0} = 0\n'.format(j))
            ...
            file.write('b{0}t{0} - a{0}u{0} = 0\n'.format(j))
            file.write('d{0} - t{0} +u{0} = 0\n'.format(j))
            file.write('d{0} >= 0\n'.format(j))
        ...
        for i in range(self.network.num_flows):
            ...
            for (k, j) in enumerate(self.network.flows[i].path):
                if k > 0 and self.network.servers[self.network.flows[i].path[k - 1]].regulator > 0:
                    file.write("x{0}l0s{1} - x{0}l0s{2} = 0\n".format(i, j, reg[k]))

    @property
    def solve(self) -> int: # Tuple[np.ndarray, defaultdict, defaultdict]:
        """
    ...
    def single_server_backlog(self, server: int) -> float:
        ...
        for i in ff.flows_in_server[server]:
            for k, tb in enumerate(ff.flows[i].arrival_curve):
                file.write('f{0} - {2} u<= {1}\n'.format(i, tb.sigma, tb.rho))
        for (j, list_flows, shaping) in ff.arrival_shaping:
            if j == server:
                #file.write("0")
                for tb in shaping:
                    file.write('f{0}'.format(list_flows[0]))
                    for i in list_flows[1:]:
                        file.write('+ f{0}'.format(i))
                    file.write('- {1} u<= {0}\n'.format(tb.sigma, tb.rho))
        # file.write("0")
        file.write("f{0}".format(ff.flows_in_server[server][0]))
        for i in ff.flows_in_server[server][1:]:
            file.write(" + f{0}".format(i))
        file.write(" -a= 0\n")
