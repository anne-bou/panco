...
def delta_from_deadlines(deadlines: List[float]):
    n = len(deadlines)
    return [[deadlines[j] - deadlines[i] for j in range(n)] for i in range(n)]


class EdfSinkTreeTFA:
    ...
    def tfa_analysis(self):
        ...
        for j in range(self.network.num_servers):
            ...
            for i in self.network.flows_in_server[j]:
                delays_server[i] = self.tfa_server(j, i)
...
# DEADLINES = [5, 7, 7]
#
# edf = EdfSinkTreeTFA(NET, DEADLINES)
#
# DELAYS = edf.tfa_analysis()
# for i in range(NET.num_flows):
#     print(DEADLINES[i], DELAYS[i])
# #print(edf.check_deadlines())
#
#
